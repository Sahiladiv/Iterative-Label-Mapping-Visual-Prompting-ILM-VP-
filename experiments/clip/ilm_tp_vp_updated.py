...
if __name__ == '__main__':
    ...
    for epoch in range(args.epoch):
        ...
        for i, (x, y) in enumerate(pbar):
            ...
            # Unscale gradients and clip them to avoid exploding gradients
            scaler.unscale_(optimizer)
            torch.nn.utils.clip_grad_norm_(visual_prompt.parameters(), max_norm=1.0)
            
            # Optimizer step and scaler update
            scaler.step(optimizer)
            scaler.update()
            
            # Clamp logit scale to avoid instability
            with torch.no_grad():
                model.logit_scale.clamp_(0, 4.6052)
            
            # Update metrics
            total_num += y.size(0)
            true_num += torch.argmax(fx, dim=1).eq(y).float().sum().item()
